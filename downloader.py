...
def reconnect_and_sleep_after_fetched_max_files():
    ...
    if url_cnt % MAX_PER_IP == 0:
        ...
        reconnect_net()
        t = SLEEP_AFTER_DOWNLOAD_MAX_FILE + randint(0, 100) / 10  # 0.0s ~ 10.0s
        logging.log(MyINFO, 'sleep for %d seconds' % t)
        time.sleep(t)
...
def sleep_after_visit(file_name):
    ...
    logging.log(MyINFO, "sleep for %.1f seconds when fetching file info [ %s ]" % (sleep_time, file_name))
...
def sleep_after_banned(message):
    ...
    BANNED += 1
    reconnect_net()
    _SLEEP_AFTER_BANNED = SLEEP_AFTER_BANNED + randint(0, 600) / 10  # 0.0s ~ 60.0s
    logging.log(MyINFO, '%d th benned when %s, sleep for %d second' % (BANNED, message, _SLEEP_AFTER_BANNED))
    time.sleep(_SLEEP_AFTER_BANNED)
    ...
    if NEED_DOWNLOAD:
        ...
        if not os.path.exists(os.getcwd() + '\\' + file_name):
            ...
            try:
                    ...
                    for chunk in downloaded_file.iter_content(chunk_size=block_size):
                        if chunk:  # filter out keep-alive new chunks
                            ...
                            sys.stdout.write("\r.......%d%%, %d KB, %d KB/s, %d seconds passed" %
                                             (percent, progress_size / KB, speed, duration))
                            file.write(chunk)
                            file.flush()
                ...
                logging.log(MyINFO, "file download and saved using %.2f seconds" % (end - start))
            except requests.RequestException as e:
                logging.exception('except: %s' % e)
    ...
    try:
        with connection.cursor() as cursor:
            ...
            for r in result:
                ...
                cnt_update += 1
                if (cnt_update % 10 == 0):
                    logging.log(MyINFO, "%d urlInfo UPDATED" % cnt_update)
    finally:
        connection.close()
...
SLEEP_L = 5.0  # s
SLEEP_H = 15.0  # s
MAX_PER_IP = 100  # times
SLEEP_AFTER_BANNED = 90.0  # s
SLEEP_AFTER_DOWNLOAD_MAX_FILE = 30.0  # s
####
TOTAL = 1000
NEED_UPDATE = False
NEED_DOWNLOAD = False
